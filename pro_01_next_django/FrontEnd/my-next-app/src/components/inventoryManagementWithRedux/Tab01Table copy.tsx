...
import { InventoryItemExport, formatNumber } from '@/features/formReceiptSlip/inventoryTableSlice';
...
export function InventoryTableStockReceiveSlip({ product, onInventoryTableChange }: InventoryTableStockReceiveSlipProps) {
  ...
  const addRow = () => {
    ...
    // Create new item with formatted values
    const newItem: InventoryItemExport = {
      ...
      notes: product.notes || "",
      rawQuantity: formatNumber(product.quantity),
      rawPrice: formatNumber(product.price),
    };
  ...
  const handleInputChange = (
    id: number,
    field: 'rawQuantity' | 'rawPrice',
    value: string
  ) => {
    dispatch(setRawValue({ id, field, value }));
  };

  // Xử lý blur để định dạng và cập nhật state
  const handleInputBlur = (
    ...
    handleUpdateItem(id, field, finalValue);
  };
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => handleInputChange(item.id, "rawQuantity", e.target.value)}
                        onBlur={(e) => handleInputBlur(item.id, "quantity", e.target.value)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                          }
                    ...
                    <td>
                      <input
                        ...
                        className="form-control form-control-sm"
                        value={item.rawPrice}
                        onChange={(e) => handleInputChange(item.id, "rawPrice", e.target.value)}
                        onBlur={(e) => handleInputBlur(item.id, "price", e.target.value)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                          }
