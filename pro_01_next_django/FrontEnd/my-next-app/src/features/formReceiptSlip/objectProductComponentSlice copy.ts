...
// ==== 4. Hàm tiện ích để định dạng số (123,123,122 hoặc 13,123,122.13) ====
const formatNumberDisplay = (value: number): string => {
  const stringValue = value.toFixed(2); // Đảm bảo luôn có 2 chữ số thập phân
...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    setQuantity: (state, action: PayloadAction<string>) => {
      ...
      state.value = formatNumberDisplay(state.inventoryItem.value);
    },
...
// ==== 7. Export reducer để dùng trong store ====
