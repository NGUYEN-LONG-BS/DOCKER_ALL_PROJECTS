...
class PA_InventoryStockSerializer(serializers.ModelSerializer):
    ...
    def get_ten_hang(self, obj):
        ...
        category = PA_INVENTORY_CATEGORIES.objects.using(DB_NAME_TB).filter(ma_hang=obj.ma_hang).first()
        return category.ten_hang if category else 'N/A'

    def get_ten_doi_tuong(self, obj):
        category = PA_SUPPLIER_CATEGORIES.objects.using(DB_NAME_TB).filter(ma_nha_cung_cap=obj.ma_doi_tuong).first()
