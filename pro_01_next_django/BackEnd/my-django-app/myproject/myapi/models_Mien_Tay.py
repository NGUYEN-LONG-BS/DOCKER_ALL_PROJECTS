...
class MIENTAY_INVENTORY_CATEGORIES(models.Model):
    ...
    class Meta:
        db_table = 'inventory"."MIENTAY_INVENTORY_CATEGORIES'  # Chỉ định schema, tạo bảng trước, sau đó đổi lại tên
...
class MIENTAY_INVENTORY_STOCK_RECEIVED_ISSSUED_RETURNED(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    date = models.DateTimeField(auto_now_add=True)
    id_nhan_vien = models.CharField(max_length=10, null=False, blank=False)
    xoa_sua = models.CharField(max_length=10, null=False, blank=False)
    ...
    class Meta:
        db_table = 'inventory"."MIENTAY_INVENTORY_STOCK_RECEIVED_ISSSUED_RETURNED'  # Chỉ định schema, tạo bảng trước, sau đó đổi lại tên
        managed = True  # Django sẽ quản lý bảng này hoàn toàn (tạo, xóa, sửa theo model).
...
class MIENTAY_SUPPLIER_CATEGORIES(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    date = models.DateTimeField(auto_now_add=True)
    id_nhan_vien = models.CharField(max_length=10, default="")
    xoa_sua = models.CharField(max_length=10, default="")
    ...
    def is_duplicate(ma_nha_cung_cap):
        return MIENTAY_SUPPLIER_CATEGORIES.objects.filter(ma_nha_cung_cap=ma_nha_cung_cap, xoa_sua="new").exists()
